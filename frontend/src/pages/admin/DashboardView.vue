...
function getMonthlyIncomeBasicArea(data) {
  const { setOption, resize } = monthlyIncomeBasicArea(monthlyIncomeBasicAreaDom.value)
  setOption(data)
  window.addEventListener('resize', () => {
    resize()
  })
...
<template>
  <div>
    ...
    <p class="mt-2 text-lg text-primary-300 leading-relaxed">即時掌握方案銷售、預約狀況與營收表現</p>
  </div>
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-12 gap-6 mb-8">
    <div class="bg-primary-800 rounded-4xl border-4 border-primary-600 p-5 xl:col-span-3">
    ...
    <div class="bg-primary-800 rounded-4xl border-4 border-primary-600 p-5 xl:col-span-3">
      <div class="flex items-center justify-between h-full">
        <div>
          ...
          <p class="text-lg font-semibold text-primary-0 mt-1">本月活躍用戶數</p>
        </div>
        <div class="flex items-center text-red-400">
          <span class="text-2xl font-bold">150</span>
          <span class="text-2xl font-bold">↑</span>
    ...
    <div class="bg-primary-800 rounded-4xl border-4 border-primary-600 p-5 md:col-span-2 xl:col-span-6 relative">
