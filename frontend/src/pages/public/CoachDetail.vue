...
import { CoachAPI, CourseAPI } from '@/api/index.js'
...
const redirectModalTable = {
  success: () => openRedirectModal('報名成功！', '前往課程儀表板', '/user/dashboard'),
  已經報名過此課程: () => openRedirectModal('您已報名過此課程！', '前往課程儀表板', '/user/dashboard'),
  已無可使用堂數: () => openRedirectModal('您購買堂數不足請先續約健身方案', '前往健身方案', '/fitness-plans'),
  '已達最大參加人數，無法參加': () => openRedirectModal('已達最大參加人數，無法參加', '前往尋找教練', '/coaches'),
  請先登入: () => openRedirectModal('您尚未登入，請先登入以繼續操作。', '前往登入頁面', '/login'),
}
...
async function confirmRegistration(id) {
  try {
    const { status } = await CourseAPI.postCourse(id)
...
async function getCoachInfo() {
  try {
    const id = getCoachId()
    const { data } = await CoachAPI.getCoachDetail(id)
...
async function getCoachCourseList() {
  try {
    const id = getCoachId()
    const { data } = await CoachAPI.getCoachCourses(id)
...
<template>
  <div class="bg-primary-900 min-h-screen py-12 md:py-16 lg:py-20">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 md:px-8 2xl:px-0">
      <div v-if="errorMessage" class="flex items-center justify-center" :style="{ minHeight: contentHeight }">
        <div class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded text-3xl">
          {{ errorMessage }}
        </div>
      </div>
      <div v-if="!errorMessage" class="flex flex-col md:flex-row gap-6 md:gap-12 lg:gap-16 items-center md:items-start">
        <img
        ...
        <div class="flex-1 w-full text-center md:text-left">
          ...
          <h5 class="text-2xl sm:text-3xl font-bold text-primary-0 mb-3 md:mb-4">擅長領域</h5>
          <ul class="flex flex-wrap gap-3 sm:gap-4 md:gap-6 justify-center md:justify-start mb-4 md:mb-6">
            <li
      ...
      <div v-if="!errorMessage" class="mt-12 md:mt-16">
        <h5 class="text-2xl sm:text-3xl font-bold text-primary-0 mb-6 md:mb-8">最近開課</h5>
          ...
          >
            <div class="p-4 sm:p-5 md:p-6">
              <div class="flex items-start justify-between mb-4 gap-2">
                <h6 class="text-xl sm:text-2xl md:text-3xl font-bold text-primary-0 flex-1">
                  {{ course.name }}
                </h6>
                <span
              ...
              <p class="text-sm sm:text-base md:text-lg text-primary-300 mb-4 sm:mb-5 md:mb-6 line-clamp-2">
                {{ course.description }}
              </p>
              ...
              <div class="space-y-2 sm:space-y-3">
                <div class="flex items-center text-primary-300">
                  ...
                  <span class="text-sm sm:text-base md:text-lg whitespace-nowrap">
                    <span class="font-bold">課程時間：</span>{{ formatCourseDateTime(course.start_at, course.end_at) }}
                  </span>
                </div>
                ...
                <div class="flex items-center text-primary-300">
                  ...
                  <span class="text-sm sm:text-base md:text-lg"
                    ><span class="font-bold">名額：</span>{{ course.max_participants }} 人</span
                  >
                </div>
              </div>

              <button
                @click="openConfirmWhetherToRegisterModal(course.coach_name, course.name, course.id)"
