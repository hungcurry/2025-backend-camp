...
import { CoachAPI, AdminAPI } from '@/api/index.js'
import { formatLocalDate, formatTimeRange, toUTC } from '@/utils/formatDateTime.js'
...
const emit = defineEmits(['closeCourseModal', 'submitCourse', 'addCourse', 'updateCourse'])
...
async function getCoachCourseInfo() {
  try {
    const { data } = await AdminAPI.getCoachCourse(props.courseId)
    ...
    >
      ...
      <div class="space-y-6">
        ...
        <div>
          ...
          <input
            id="meetingUrl"
            v-model="courseInfo.meeting_url"
            type="url"
            required
            class="w-full px-4 py-3 text-lg border-2 border-primary-600 bg-primary-700 text-primary-0 rounded-lg focus:outline-none focus:border-secondary-800 transition-all duration-300 placeholder-primary-400"
        ...
        <div>
          ...
          <div class="relative group">
            ...
            />
            <svg
              class="absolute right-3 top-1/2 -translate-y-1/2 w-6 h-6 text-primary-0 pointer-events-none"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
        ...
        <div>
          ...
          <input
            id="maxParticipants"
            v-model.number="courseInfo.max_participants"
            type="number"
            min="1"
            required
            class="w-full px-4 py-3 text-lg border-2 border-primary-600 bg-primary-700 text-primary-0 rounded-lg focus:outline-none focus:border-secondary-800 transition-all duration-300 placeholder-primary-400"
